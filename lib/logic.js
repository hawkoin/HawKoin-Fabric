...
async function transferFunds(trade) {
    ...
    if (trade.amount <= 0) {
        throw new Error('Transaction failed. Stop trying to steal money!');
    }
    ...
    if (demoMode) {
        ...
        txnAuthenticated = await verifyOAuth(trade.fromUser.contactInfo.email, trade.authToken);
    }
    ...
    if(rb < trade.fromUser.lowBalThreshold) {
