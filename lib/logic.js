...
async function transferFunds(trade) {
  	...
  	//Gets the network admin who is running the network (aka the current user)
  	//let currentParticipantId = getCurrentParticipant().participantId; 
    
  	console.log(trade.fromUser.$type);
  	console.log(trade.fromUser.id);
 	...
 	var txnAuthenticated = false;
  	...
  	//If the network admin is a demoadmin, skip oauth. Important for unit tests and scripts.
  	if (demoMode) {
